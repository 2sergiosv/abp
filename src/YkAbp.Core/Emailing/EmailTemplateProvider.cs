...
namespace YkAbp.Core.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ITransientDependency
    {
        ...
        {
            using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("YkAbp.Core.Emailing.EmailTemplates.default.html"))
            {
