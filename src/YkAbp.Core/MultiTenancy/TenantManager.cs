...
using YkAbp.Core.MultiTenancy.Demo;
...
namespace YkAbp.Core.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            ...
            var error = LocalizationManager.GetSource(YkAbpConsts.LocalizationSourceName).GetString("FreeEditionsCannotHaveTrialVersions");
        ...
        public decimal GetUpgradePrice(SubscribableEdition currentEdition, SubscribableEdition targetEdition, int remainingDaysCount)
        {
            ...
            if (remainingDaysCount > (int)EPaymentPeriodType.Annual)
            {
                var remainingsYearsCount = remainingDaysCount / (int)EPaymentPeriodType.Annual;
                remainingDaysCount = remainingDaysCount % (int)EPaymentPeriodType.Annual;
        ...
        private static decimal GetMonthlyCalculatedPrice(SubscribableEdition currentEdition, SubscribableEdition upgradeEdition, int remainingDaysCount)
        {
            ...
            {
                currentUnusedPrice = (currentEdition.MonthlyPrice.Value / (int)EPaymentPeriodType.Monthly) * remainingDaysCount;
            }

            if (upgradeEdition.MonthlyPrice.HasValue)
            {
                upgradeUnusedPrice = (upgradeEdition.MonthlyPrice.Value / (int)EPaymentPeriodType.Monthly) * remainingDaysCount;
            }
